...
void APlatformerHUD::ShowProfile(FString profile)
{
	RuyiUserProfile = profile;
}
...
void APlatformerHUD::DrawHighscore()
{
	...
	//TextItem.Scale = FVector2D( TextScale * UIScale, TextScale * UIScale );
	TextItem.Scale = FVector2D(TextScale * UIScale * 0.5f, TextScale * UIScale * 0.5f);
	TextItem.EnableShadow( FLinearColor::Transparent );
	Canvas->DrawItem( TextItem );
...
void APlatformerHUD::DrawWaiting()
{
	const FText TxtTitle = LOCTEXT("Title", "Network Request");
	const float SizeX = Canvas->ClipX * 0.6f;
	const float SizeY = 1000 * UIScale;
	const float DrawX = (Canvas->ClipX - SizeX) / 2.0f;
	const float DrawY = (Canvas->ClipY - SizeY) / 2.0f;
	...
	const float TextMargin = 0.03f;
	float StrSizeX, StrSizeY;
	Canvas->StrLen(HUDFont, TxtTitle.ToString(), StrSizeX, StrSizeY);
	StrSizeX = StrSizeX * TextScale * UIScale;
	StrSizeY = StrSizeY * TextScale * UIScale;

	FCanvasTextItem TextItem(FVector2D((Canvas->ClipX - StrSizeX) / 2.0f, DrawY + SizeY * TextMargin),
		TxtTitle, HUDFont, FLinearColor::White);
	TextItem.Scale = FVector2D(TextScale * UIScale, TextScale * UIScale);
	TextItem.EnableShadow(FLinearColor::Transparent);
	Canvas->DrawItem(TextItem);

	const FText TxtInfo = LOCTEXT("Info", "Please Waiting...");
	TextItem.Text = TxtInfo;
	TextItem.Position = FVector2D((Canvas->ClipX - StrSizeX) / 2.0f, (Canvas->ClipY - SizeY * 0.5f) / 2.0f);
	Canvas->DrawItem(TextItem);
}

void APlatformerHUD::DrawProfile()
{
	const FText profile = FText::FromString(RuyiUserProfile);
	//const FText profile = FText::Format(LOCTEXT("Title", "%s"), RuyiUserProfile);
	const float SizeX = Canvas->ClipX * 0.2f;
	const float SizeY = 1000 * UIScale * 0.2f;
	const float DrawX = (Canvas->ClipX - SizeX) * 0;
	const float DrawY = (Canvas->ClipY - SizeY) * 0;
...
void APlatformerHUD::DrawLeadboard()
{
...
void APlatformerHUD::EndHighscoreEntryPrompt()
{
	bEnterNamePromptActive = false;
	if (PlayerOwner)
	{
...
void APlatformerHUD::UpdateHighscoreNamesAndScores(TArray<FRuyiLeaderBoardScore>& highscoreData)
{
	for(int i = 0; i < highscoreData.Num(); ++i)
	{
		for(int j = i; j < highscoreData.Num(); ++j)
		{
			if (highscoreData[i].score > highscoreData[j].score)
			{
				highscoreData.Swap(i, j);
			}
		}
	}
	...
	for (int32 i = 0; i < 10; i++)
	{
		if (i < highscoreData.Num())
		{
			HighscoreTimes[i] = highscoreData[i].score;
			HighscoreNames[i] = highscoreData[i].name;
		} else 
		{
			HighscoreTimes[i] = 0;
			HighscoreNames[i] = "NoName";
		}
	}
